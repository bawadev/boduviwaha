...
export const logoutAndReset = createAction("userDetail/logoutAndReset");

const userDetailSlice = createSlice({
  name: "userDetail",
  initialState: initialState,
  reducers: {
    updateUserImageVisibility: (state, action) => {
      state.userImageVisibility = action.payload.userImageVisibility;
    },
    updateUserProfileImage: (state, action) => {
      state.userProfileImage = action.payload.userProfileImage;
    },
    updateUserImages: (state, action) => {
      const { image, imageId, actionType } = action.payload;

      if (actionType === "ADD") {
        // Add new images to the existing array
        state.userImages.images.push(image);
      } else if (actionType === "ADD_ALL") {
        // Add multiple new images to the existing array
        state.userImages.images.push(...image);
      } else if (actionType === "DELETE_ONE") {
        // Remove a particular image by its id
        state.userImages.images = state.userImages.images.filter(
          (img) => img.id !== imageId
        );
      ...
      // Optionally, update isLoaded state
      state.userImages.isLoaded = true;
    },
  ...
  extraReducers: (builder) => {
    builder.addCase(logoutAndReset, (state) => {
